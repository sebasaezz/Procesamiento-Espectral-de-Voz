...
def ola(X, M, hop, A_max, C = 1):
    
    # primero, calculamos en cada bloque la inversa

    if X is None:
        return None

    bloques_reconstruidos = np.fft.ifft(X, axis = 1)
    bloques_reconstruidos = np.real(bloques_reconstruidos)

    #asegurtarse de volver al largo
    ...
    reconstruccion = reconstruccion * A_max
    reconstruccion = reconstruccion/C

    return reconstruccion

def reconstruir_audio(x, u_s, M, nombre_ventana, nombre_audio):
    if x is None:
